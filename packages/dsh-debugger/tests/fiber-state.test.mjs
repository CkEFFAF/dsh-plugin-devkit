/**
 * Guard test: the hardcoded FiberState mirror must match real cordis source.
 *
 * Cordis declares `export const enum FiberState`, which is erased at runtime, so
 * the values cannot be imported and must be mirrored as literals in
 * `src/fiber-state.mjs`. Design document 4.5 requires that drift be a *build
 * failure* rather than a cosmetic "Unknown", so this test parses the real source
 * file and compares member order and values.
 *
 * If this test fails after a cordis upgrade, re-read the enum and update
 ...
 *
 * Handles both `const enum` and plain `enum`, and skips comments/members with
...
export function parseFiberStateEnum(source) {
  ...
  const body = match[1]
  // Strip line comments and block comments before parsing members.
  const cleaned = body
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/\/\/[^\n]*/g, '')
  ...
  for (const raw of cleaned.split(',')) {
    const text = raw.trim()
    if (!text) continue
    const [namePart, valuePart] = text.split('=').map((s) => s.trim())
    if (!/^[A-Za-z_$][\w$]*$/.test(namePart)) continue
    const value = valuePart === undefined || valuePart === ''
      ? auto
      : Number(valuePart)
...
test('fiber-state mirror matches cordis source order and values', (t) => {
  const source = findCordisSource()
  if (!source) {
    // Honest skip: without the checkout the guard cannot prove anything, and a
    // silent pass would be exactly the false confidence the guard exists to stop.
...
test('the enum is recognised as a const enum (hence runtime-erased)', (t) => {
  ...
  // This documents *why* the mirror exists. If cordis ever ships a runtime enum,
  // this test fails and tells the author to import it instead of mirroring.
  assert.match(
    text,
    /export\s+const\s+enum\s+FiberState/,
    'cordis now exports a runtime FiberState enum — import it and delete the mirror in src/fiber-state.mjs',
  )
})
